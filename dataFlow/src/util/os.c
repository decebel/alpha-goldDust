...
UINT64 waitUntil(UINT64 nexttimeInUs)
	...
	UINT64 waitTime, waitTimeInMSec;
...
UINT64 getCpuUsageInUs()
	...
	UINT64 usage =
		...
		(UINT64) (resUsage.ru_stime.tv_sec) * 1000000LL +
		(UINT64) (resUsage.ru_stime.tv_usec);
	return usage;
#else
	return 0;
#endif
...
void getUsageTime(long long *userTime_p, long long *systemTime_p)
	...
	*systemTime_p =
		(long long) usage.ru_stime.tv_sec * (long long) 1000000 +
...
BOOL condTimedWait(CONDITION_T * cond_p, MUTEX_T * mutex_p, int timeoutSec      /* negative means forever */
	...
	waitResult = SignalObjectAndWait(*cond_p, *mutex_p,
		waitInterval, FALSE);
